...
print('uti dir ', os.environ.get('ARA_UTIL_INSTALL_DIR'))
#ROOT.gSystem.Load('/home/pgiri/analysis/AraSoft/AraUtil/lib/libAraEvent.so')
ROOT.gSystem.Load(os.environ.get('ARA_UTIL_INSTALL_DIR')+"/lib/libAraEvent.so")
#ROOT.gSystem.Load('/home/pgiri/analysis/AraSoft/AraUtil/lib/libRootFftwWrapper.so.3.0.1')
...
class ara_geom_loader:
    ...
    def get_ant_xyz(self, use_mean = False):
        ...
        for ant in range(num_useful_chs):
            print(ant,num_useful_chs)
            VPols_list = []#[0,1,2,4,5,6]
            #import New_A4_calibrated_coordinates_from_para_to_center as New_A
            if ant in VPols_list:
               get_loc = New_A.pass_st_geom()[ant]
               ant_xyz[0, ant] = get_loc[0] #- 1.5
               ant_xyz[1, ant] = get_loc[1] #+ 12.66
               ant_xyz[2, ant] = get_loc[2]
            else:
            

               ant_xyz[0, ant] = self.st_info.getAntennaInfo(ant).antLocation[0]#get_location[0][ant]#-10801.4966#self.st_info.getAntennaInfo(ant).antLocation[0]
               ant_xyz[1, ant] = self.st_info.getAntennaInfo(ant).antLocation[1]#get_location[1][ant]#-17335.5766 #self.st_info.getAntennaInfo(ant).antLocation[1]
               ant_xyz[2, ant] = self.st_info.getAntennaInfo(ant).antLocation[2]#get_location[2][ant]#self.st_info.getAntennaInfo(ant).antLocation[2]
            '''if ant == 1 or ant == 5:
               ant_xyz[2, ant] -=10
            '''
        if self.verbose:
    ...
    def get_cable_delay(self):
        ...
        for ant in range(num_useful_chs):
            VPols_list = [0,1,2,4,5,6]
            #import New_A4_calibrated_coordinates
            #if ant in VPols_list:
            #   cable_delay[ant] = New_A4_calibrated_coordinates.supply_cable_delays()[ant]
            #else:
...
class ara_sim_loader:
    ...
    def get_sub_info(self, get_angle_info = False):
        ...
        if get_angle_info:
            ...
            ROOT.gInterpreter.ProcessLine('#include "'+os.environ.get('ARA_UTIL_INSTALL_DIR1')+'/../AraSim/Report.h"')
