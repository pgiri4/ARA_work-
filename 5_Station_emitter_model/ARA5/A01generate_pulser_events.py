...
def generate_my_events(filename, n_events):
    ...
    data_sets["emitter_amplitudes"] = np.ones(n_events) * 1560 * units.V
    ...
    data_sets["xx"] = np.ones(n_events)*2100 * units.m #*21 * units.m
    data_sets["yy"] =  np.ones(n_events) *1100 * units.m #* 42 * units.m
    data_sets["zz"] = - np.ones(n_events)*1211*units.m#- np.ones(n_events) *175 * units.m#* 175 * units.m
