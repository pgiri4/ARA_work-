...
def generate_my_events(filename, n_events):
    ...
    data_sets["emitter_model"] = ["idl1"]*n_events#["ARA02_calPulser"] * n_events
    data_sets["emitter_amplitudes"] = np.ones(n_events) * 250* units.V
    ...
    data_sets["xx"] = np.ones(n_events)*34000 * units.feet #*21 * units.m
    data_sets["yy"] =  np.ones(n_events) *47500 * units.feet #* 42 * units.m
    data_sets["zz"] = - np.ones(n_events)*3100*units.feet#- np.ones(n_events) *175 * units.m#* 175 * units.m
